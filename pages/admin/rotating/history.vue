    ...
    <div class="allow_list_body">
      <div class="table_wrapper">
        <div class="add_box">
          <client-only>
            <el-date-picker v-model="daterange" type="daterange" range-separator="To"
              :start-placeholder="$t('rotating.history.startTime')" :end-placeholder="$t('rotating.history.endTime')"
              size="large" value-format="x" />
          </client-only>
          <el-input v-model="searchKey" size="large" :placeholder="$t('rotating.history.orderIdPlaceholder')"
            @keydown.enter="init">
            <template #append>
              <el-button @click="init"> {{ $t('rotating.history.searchBtn') }} </el-button>
            </template>
          </el-input>
          <el-button @click="resetHandler"> {{ $t('rotating.history.resetBtn') }} </el-button>
        </div>
        <el-table v-loading="loading" :data="tableData" :border="true" cell-class-name="list_cell_custom"
          ...
          <el-table-column prop="OrderID" :align="'center'" :label="$t('rotating.history.orderId')" />
          <el-table-column prop="Amount" :align="'center'" :label="$t('rotating.history.orderAmount')" />
          <el-table-column prop="PaymentMethod" :label="$t('rotating.history.paymentMethod')" :align="'center'" />
...
const getDataByPage = () => {
  ...
  http.get('/v1/website/payment_history/RotatingDatacenter',
    { params }
  ).then((res: any) => {
