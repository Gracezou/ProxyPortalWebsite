    ...
    <div class="allow_list_body">
      <div class="table_wrapper">
        <div class="add_box">
          ...
          <el-input v-model="searchKey" size="large" :placeholder="$t('residential.history.orderIdPlaceholder')"
            @keydown.enter="init">
            <template #append>
              <el-button @click="init"> {{ $t('residential.history.searchBtn') }} </el-button>
            </template>
          </el-input>
          <el-button @click="resetHandler"> {{ $t('residential.history.resetBtn') }} </el-button>
        </div>
...
const getDataByPage = () => {
  ...
  http.get('/v1/website/payment_history/StaticResidential',
    { params }
  ).then((res: any) => {
