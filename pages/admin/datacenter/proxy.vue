    ...
    <div class="allow_list_body">
      <div class="table_wrapper">
        <div class="add_box">
          ...
          <el-button @click="init"> {{ $t('datacenter.proxy.search') }} </el-button>
          <el-button @click="resetHandler"> {{ $t('datacenter.proxy.reset') }} </el-button>
        </div>
        <el-table v-loading="loading" :data="tableData" :border="true" cell-class-name="list_cell_custom"
          ...
          <el-table-column prop="IP" :align="'center'" :label="$t('datacenter.proxy.ip')" />
          <el-table-column prop="Port" :align="'center'" :label="$t('datacenter.proxy.port')" />
          <el-table-column prop="Region" :label="$t('datacenter.proxy.countryRegion')" :align="'center'" />
          <el-table-column prop="PaymentTime" :label="$t('datacenter.proxy.paymentTime')" :align="'center'"
            :formatter="parseTime" />
          <el-table-column prop="ExpireTime" :label="$t('datacenter.proxy.expiredTime')" :align="'center'"
            :formatter="parseTime" />
...
const getDataByPage = () => {
  ...
  http.get('/v1/website/proxy_manage/StaticDatacenter/get',
    { params }
  ).then((res: any) => {
...
const submitHandler = () => {
  loading.value = true
  http.post('/v1/website/proxy_manage/StaticDatacenter/edit', form)
    .then(() => {
