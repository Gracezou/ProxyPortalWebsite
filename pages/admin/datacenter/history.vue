    ...
    <div class="allow_list_body">
      <div class="table_wrapper">
        <div class="add_box">
          ...
          <el-input v-model="searchKey" size="large" :placeholder="$t('datacenter.history.orderIdPlaceholder')"
            @keydown.enter="init">
            <template #append>
              <el-button @click="init"> {{ $t('datacenter.history.searchBtn') }} </el-button>
            </template>
          </el-input>
          <el-button @click="resetHandler"> {{ $t('datacenter.history.resetBtn') }} </el-button>
        </div>
...
const getDataByPage = () => {
  ...
  http.get('/v1/website/payment_history/StaticDatacenter',
    { params }
  ).then((res: any) => {
